...
export const TabsList: FC<TabsListProps> = ({ tabs, onTabSelectionChange }) => {
  ...
  // };
  return (
    <ul className="flex flex-col gap-4 pt-4 max-w-[90%] mx-auto">
      {tabs?.map((tab: AppTab) => {
        const repoName = getRepoNameFromUrl(tab);
        return (
          <li
            key={tab.id}
            className="px-2 py-1 w-full border rounded-md break-anywhere whitespace-pre-wrap hover:bg-purple-700 hover:text-white cursor-pointer duration-200"
          >
            <label
              htmlFor={`tab_id_${tab.id}`}
              className="cursor-pointer w-full h-full flex items-center gap-4"
            >
              <input
                id={`tab_id_${tab.id}`}
                type="checkbox"
